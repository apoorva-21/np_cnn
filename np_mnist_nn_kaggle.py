...
test_x = test_x.T
a = forward_prop(test_x, W, b)
pred_label = np.argmax(a['OUT'], axis = 0)
pred_label = np.reshape(pred_label,(pred_label.shape[0],1))
img_id = np.arange(1,28001)
img_id = np.reshape(img_id, (img_id.shape[0],1))
output = np.hstack([img_id, pred_label])
print(output[:100])

index = []
send_dataset = pd.DataFrame({'ImageId':output[:,0], 'Label':output[:,1]})
print(send_dataset)

send_dataset.to_csv('./myOutput.csv', index = False)